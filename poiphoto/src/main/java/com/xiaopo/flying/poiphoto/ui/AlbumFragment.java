...
public class AlbumFragment extends Fragment {
      ...
      @Override public void onItemClick(View v, int position) {
        getActivity().getSupportFragmentManager()
            ...
            .replace(R.id.container,
                PhotoFragment.newInstance(mAlbumAdapter.getData().get(position)))
            .commit();
      }
    });
  ...
  private void initToolbar(Toolbar toolbar) {
    Configure configure =
        ((com.xiaopo.flying.poiphoto.ui.PickActivity) getActivity()).getConfigure();
  ...
  private class AlbumTask extends AsyncTask<Void, Integer, List<Album>> {
    ...
    @Override protected List<Album> doInBackground(Void... params) {
      return mPhotoManager.getAlbum();
    }

    @Override protected void onPostExecute(List<Album> alba) {
