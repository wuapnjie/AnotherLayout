...
public class PixelCropView extends View {
  ...
  @Override protected void onSizeChanged(int w, int h, int oldw, int oldh) {
    ...
    mCropBorder =
        new Border(new RectF(mBorderOffset, mBorderOffset, w - mBorderOffset, w - mBorderOffset));
  ...
  private void setUpDefaultCropBorder() {
    if (mCropWrapper != null) setUpCropBorder(mCropWrapper.getWidth(), mCropWrapper.getHeight());
  }
    ...
    if (mCropWrapper != null) {
      ...
      if (width >= height) {
        ...
        if ((h - bHeight) < mBorderOffset) {
          ...
          mCropBorder.setBaseRect(
              new RectF((w - bWidth) / 2, mBorderOffset, (w + bWidth) / 2, h - mBorderOffset));
        } else {
          mCropBorder.setBaseRect(
              new RectF(mBorderOffset, (h - bHeight) / 2, w - mBorderOffset, (h + bHeight) / 2));
        }
      } else {
  ...
  @Override public boolean onTouchEvent(MotionEvent event) {
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_UP:
        ...
        mPreSizeMatrix.postTranslate(currentCenterPoint.x - mDownCenterPoint.x,
            currentCenterPoint.y - mDownCenterPoint.y);
  ...
  private void handleMoveLineEvent(MotionEvent event) {
    ...
    if (mHandlingLine.getDirection() == Line.Direction.HORIZONTAL && (abs(event.getY() - mDownY)
        > mTouchSlop)) {
      mHandlingLine.moveTo(event.getY(), mBorderOffset);
    } else if (mHandlingLine.getDirection() == Line.Direction.VERTICAL && (abs(
        event.getX() - mDownX) > mTouchSlop)) {
    ...
    if (mCropWrapper != null) {
      ...
      if (mPreZoom * scale <= mMinScale) {
        postScale(mMinScale / mCropWrapper.getCurrentScale(),
            mMinScale / mCropWrapper.getCurrentScale(), mScalePoint.x, mScalePoint.y, null);

        letImageContainsBorder(0, 0, null);
      } else {
        postScale(scale, scale, mScalePoint.x, mScalePoint.y, mPreMatrix);
  ...
  private void handleDragEvent(MotionEvent event) {
    ...
    postTranslate(event.getX() - mDownX, event.getY() - mDownY, mPreMatrix);
  ...
  private void letImageContainsBorder(float preX, float preY, Matrix preMatrix) {
    if (!isImageContainsBorder()) {
      ...
      if (currentScale < mMinScale) {
        postScale(mMinScale / currentScale, mMinScale / currentScale, mCropBorder.centerX(),
            mCropBorder.centerY(), null);
      }
      ...
      postTranslate(preX + deltaX, preY + deltaY, preMatrix);
    }
  }
  ...
  private void rotate(float degrees) {
    ...
    if (mCropBorder != null) {
      ...
      postRotate(degrees, mCropBorder.centerX(), mCropBorder.centerY(), mPreSizeMatrix);
      ...
      postScale(tempScale, tempScale, mCropBorder.centerX(), mCropBorder.centerY(), null);
    }
  ...
  private void letWrapperToFitBorder() {
    ...
    if (mCropWrapper != null) {
      mCropWrapper.getMatrix().setTranslate(offsetX, offsetY);
    }
    ...
    if (mCropBorder != null) {
      ...
      mCropWrapper.getMatrix()
          .postScale(scaleX, scaleY, mCropWrapper.getMappedCenterPoint().x,
  ...
  public void cropAndSaveImage(@NonNull Bitmap.CompressFormat compressFormat, int compressQuality,
    ...
    final ImageState imageState = new ImageState(mCropBorder.getRect(),
        CropUtil.trapToRect(mCropWrapper.getMappedBoundPoints()), mCropWrapper.getCurrentScale(),
        mCropWrapper.getCurrentAngle());

    final CropParameters cropParameters =
        new CropParameters(mMaxBitmapSize, mMaxBitmapSize, compressFormat, compressQuality,
            mCropWrapper.getInputPath(), mCropWrapper.getOutputPath(), mCropWrapper.getExifInfo());

    new BitmapCropTask(((BitmapDrawable) mCropWrapper.getDrawable()).getBitmap(), imageState,
        cropParameters, cropCallback).execute();
  }
  ...
  public void setCropUri(Uri imageUri, Uri outputUri) {
    ...
    BitmapLoadUtils.decodeBitmapInBackground(getContext(), imageUri, outputUri, maxBitmapSize,
        maxBitmapSize, new BitmapLoadCallback() {

          @Override public void onBitmapLoaded(@NonNull Bitmap bitmap, @NonNull ExifInfo exifInfo,
